 ...
 */export const updateProfil = async (req, res) => {
  try {
    ...
    if (io) {
      ...
      const adminMessage = `Le profil de ${user.prenom} ${user.nom} (${user.email}) a été mis à jour par lui-même.`;
      ...
      // 📨 Notification persistante pour l'utilisateur lui-même (facultatif)
      await Notification.create({
  ...
  try {
    ...
    if (io) {
      ...
      await Notification.create({
        ...
        link: `/profil`  // ou `/parametres` selon votre frontend
      });
